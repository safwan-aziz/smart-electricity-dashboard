...
st.title("⚡ Smart Electricity Monitoring (Live)")
...
if st.session_state.running:
    ...
    while st.session_state.running:
        ...
        with placeholder_metrics.container():
            st.metric("Latest Reading", f"{usage} kWh")
            st.metric("Total Usage", f"{round(total_usage,3)} kWh")
            st.metric("Estimated Bill", f"₹{bill}")

            if total_usage > DAILY_LIMIT:
                st.error("⚠ Daily limit exceeded!")

        # Smooth live graph using line_chart
